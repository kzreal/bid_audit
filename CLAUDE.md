...
**Users**: 投标专员，投标截止前紧急审核，时间压力大，需快速定位问题。熟悉业务但对技术不敏感。

**Brand Personality**: 专业、高效、克制。工具化界面，减少认知负担。

**Aesthetic Direction**:
- Vercel 极简：大量留白、1px 细边框、2px 圆角
- 主色 Vercel 蓝 (#0070f3)，状态色仅用于审核结论
- 浅色模式，黑灰为主

**Design Principles**:
1. 减少步骤，合并流程（相关操作同一视图）
2. 状态清晰，反馈即时（spinner、进度条）
3. 视觉层级分明（18px/14px/12px 三级）
4. 克制用色（黑白灰 + 蓝色强调 + 红绿黄仅状态）
5. 信息密度优先（关键突出，次要折叠）
